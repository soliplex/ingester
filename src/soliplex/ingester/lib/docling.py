...
async def docling_convert(
    ...
    async with _http_sem:
        ...
        if "status" not in res:
            raise ValueError(
                f"no status in response: {res}"
            )
        logger.info(
            f"{task_id} result={res['status']} "
            f"processing time={res['processing_time']}"
        )
