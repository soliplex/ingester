"""
Authentication utilities for Soliplex Ingester.
...
async def require_auth(user: AuthenticatedUser = Depends(get_current_user)) -> AuthenticatedUser:  # noqa: B008
    """
    Stricter dependency that always requires authentication.

    Unlike get_current_user, this rejects anonymous users even when auth is disabled.
    Useful for sensitive endpoints that should always be protected.
    """
    if user.method == "none":
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
